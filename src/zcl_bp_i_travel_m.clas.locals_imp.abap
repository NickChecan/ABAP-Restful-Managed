...
*      IMPORTING keys FOR Travel~CalculateTravelKey.
    ...
    FOR DETERMINATION Travel~CalculateTravelKey
...
*      IMPORTING keys FOR Travel~validateCustomer.
    ...
    FOR VALIDATION Travel~validateCustomer
    IMPORTING keys FOR Travel.
...
*      IMPORTING keys FOR ACTION Travel~acceptTravel RESULT result.
    ...
    FOR MODIFY IMPORTING keys
    FOR ACTION Travel~acceptTravel
    RESULT result.
...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD CalculateTravelKey .
    ...
    LOOP AT keys INTO DATA(ls_key).
      ...
      MODIFY ENTITIES OF zi_travel_m  IN LOCAL MODE ENTITY Travel
  ...
  METHOD validate_customer.
    ...
    READ ENTITY zi_travel_m\\travel FROM VALUE #(
    ...
    DATA lt_customer TYPE SORTED TABLE OF /dmo/customer
    WITH UNIQUE KEY customer_id.

* Optimization of DB select: extract distinct non-initial customer IDs
    lt_customer = CORRESPONDING #(
        lt_travel DISCARDING DUPLICATES MAPPING customer_id = customer_id EXCEPT *
    ).

    DELETE lt_customer
    WHERE customer_id IS INITIAL.
    ...
    LOOP AT lt_travel_result INTO DATA(ls_travel_result).
        ...
        ELSEIF ls_travel_result-begin_date < cl_abap_context_info=>get_system_date( ). " begin_date must be in the future
            ...
            APPEND VALUE #(
                %key = ls_travel_result-%key
                %msg = new_message(
                    id       = /dmo/cx_flight_legacy=>begin_date_before_system_date-msgid
...
* Read changed data for action result
    READ ENTITIES OF zi_travel_m IN LOCAL MODE ENTITY travel
    FROM VALUE #(
        FOR key IN keys (
            mykey = key-mykey
            %control = VALUE #(
...
*   %control-<fieldname> specifies which fields are read from the entities
    ...
    READ ENTITY zi_travel_m FROM VALUE #(
        FOR keyval IN keys (
            %key                    = keyval-%key
